...
function renderStaticText() {
  ...
  $("voltage-chart-eyebrow").textContent = t("voltageChartEyebrow");
...
function createChart(id, config) {
  ...
  state.charts[id] = new window.Chart(ctx, config);
}
...
function voltageChartConfig(result) {
  const labels = [
    segmentLabel("inner_oil_film"),
        ...
        datasets: [
          {
            label: "Case A",
            data: [
              result.case_a.summary.mean_inner_voltage_ratio * 100,
          ...
          },
          {
            label: "Case B",
            data: [
              result.case_b.summary.mean_inner_voltage_ratio * 100,
  ...
  return {
    ...
    data: {
      ...
      datasets: [
        {
          label: t("voltageChartTitle"),
          data: [
            result.summary.mean_inner_voltage_ratio * 100,
...
function fieldChartConfig(result) {
  const labels = [
    segmentLabel("inner_oil_film"),
    segmentLabel("ceramic_ball"),
    segmentLabel("outer_oil_film")
...
function frequencyChartConfig(result) {
  const buildPoints = (caseResult) =>
    caseResult.frequency_sweep.frequency_hz.map((frequency, index) => ({
      x: frequency,
      y: caseResult.frequency_sweep.impedance_magnitude_ohm[index]
    }));
  ...
  const datasets =
    state.mode === "compare"
      ? [
          {
            label: "Case A",
            data: buildPoints(result.case_a),
            borderColor: "rgba(15, 109, 117, 1)",
            backgroundColor: "rgba(15, 109, 117, 0.18)",
            pointRadius: 0,
            borderWidth: 2
          },
          {
            label: "Case B",
            data: buildPoints(result.case_b),
            borderColor: "rgba(196, 109, 45, 1)",
            backgroundColor: "rgba(196, 109, 45, 0.18)",
            pointRadius: 0,
            borderWidth: 2
          }
        ]
  ...
  return {
    ...
    options: {
      ...
      scales: {
        x: {
          type: "logarithmic",
          title: { display: true, text: t("frequencyXAxis") }
        },
        y: {
          type: "logarithmic",
          title: { display: true, text: t("frequencyYAxis") }
        }
      }
    }
  };
...
function sweepSource(caseResult) {
  return caseResult.condition_sweeps[state.sweepVariable];
}

function sweepMetricLabel(metricKey) {
  if (metricKey === "effective_capacitance_pf") {
    return t("metricTotalCapacitance");
  }
  if (metricKey === "max_oil_film_field_mv_m") {
    return t("metricMaxField");
  }
  return t("metricRiskNumeric");
}

function sweepXAxisLabel() {
  if (state.sweepVariable === "speed") {
    return t("sweepXAxisSpeed");
  }
  if (state.sweepVariable === "temperature") {
    return t("sweepXAxisTemperature");
  }
  return `${t("sweepXAxisLoad")} (${state.units.load})`;
}

function sweepYLabel() {
  if (state.sweepMetric === "effective_capacitance_pf") {
    return state.units.capacitance === "nF" ? "nF" : "pF";
  }
  if (state.sweepMetric === "max_oil_film_field_mv_m") {
    return state.units.field === "V/um" ? "V/μm" : "MV/m";
  }
  return t("riskLevel");
}

function convertSweepMetricValue(metricKey, value) {
  if (metricKey === "effective_capacitance_pf" && state.units.capacitance === "nF") {
    return value / 1000;
  }
  return value;
}

function convertSweepX(value) {
  if (state.sweepVariable === "radial_load" && state.units.load === "kN") {
    return value / 1000;
  }
  return value;
}

function sweepChartConfig(result) {
  const sourceA = sweepSource(state.mode === "compare" ? result.case_a : result);
  const buildDataset = (source, label, color) => ({
    label,
    data: source.x.map((xValue, index) => ({
      x: convertSweepX(xValue),
      y: convertSweepMetricValue(state.sweepMetric, source[state.sweepMetric][index])
    })),
    borderColor: color,
  ...
  const datasets =
    state.mode === "compare"
      ? [
          buildDataset(sourceA, "Case A", "rgba(15, 109, 117, 1)"),
          buildDataset(sweepSource(result.case_b), "Case B", "rgba(196, 109, 45, 1)")
        ]
      : [buildDataset(sourceA, sweepMetricLabel(state.sweepMetric), "rgba(15, 109, 117, 1)")];
  ...
  return {
    ...
    options: {
      ...
      scales: {
        x: {
          type: "linear",
          title: { display: true, text: sweepXAxisLabel() }
        },
        y: {
          type: "linear",
          title: { display: true, text: sweepYLabel() }
        }
      }
    }
  };
...
function sensitivityDelta(entry, metricKey) {
  if (metricKey === "effective_capacitance_pf") {
    return Math.max(
      Math.abs(entry.plus_10.effective_capacitance_pf - entry.baseline.effective_capacitance_pf),
      Math.abs(entry.minus_10.effective_capacitance_pf - entry.baseline.effective_capacitance_pf)
    );
  }
  if (metricKey === "max_oil_film_field") {
    return Math.max(
      Math.abs(entry.plus_10.max_oil_film_field_mv_m - entry.baseline.max_oil_film_field_mv_m),
      Math.abs(entry.minus_10.max_oil_film_field_mv_m - entry.baseline.max_oil_film_field_mv_m)
    );
  }
  return Math.max(
    Math.abs(entry.plus_10.risk_level_numeric - entry.baseline.risk_level_numeric),
    Math.abs(entry.minus_10.risk_level_numeric - entry.baseline.risk_level_numeric)
  );
}

function sensitivityChartConfig(result) {
  const buildRankingMap = (caseResult) => {
    const rows = caseResult.sensitivity[state.sensitivityMetric];
    return Object.fromEntries(rows.map((entry) => [entry.parameter, sensitivityDelta(entry, state.sensitivityMetric)]));
  };

  const mapA = buildRankingMap(state.mode === "compare" ? result.case_a : result);
  const mapB = state.mode === "compare" ? buildRankingMap(result.case_b) : null;
  const labels = Object.keys(mapA).sort((left, right) => {
    const leftValue = Math.max(mapA[left] || 0, mapB?.[left] || 0);
    const rightValue = Math.max(mapA[right] || 0, mapB?.[right] || 0);
  ...
  const datasets =
    state.mode === "compare"
      ? [
          {
            label: "Case A",
            data: labels.map((label) => mapA[label] || 0),
            backgroundColor: "rgba(15, 109, 117, 0.75)"
          },
          {
            label: "Case B",
            data: labels.map((label) => mapB?.[label] || 0),
            backgroundColor: "rgba(196, 109, 45, 0.75)"
          }
        ]
      : [
          {
            label: t("sensitivityChartTitle"),
            data: labels.map((label) => mapA[label] || 0),
            backgroundColor: "rgba(15, 109, 117, 0.75)"
          }
        ];

  let xTitle = t("sensitivityXAxisCap");
  if (state.sensitivityMetric === "max_oil_film_field") {
    xTitle = t("sensitivityXAxisField");
  } else if (state.sensitivityMetric === "risk_level") {
    xTitle = t("sensitivityXAxisRisk");
  }

  return {
    type: "bar",
    data: {
      labels: labels.map((label) => parameterLabel(label)),
      datasets
    },
    options: {
      indexAxis: "y",
  ...
  try {
    ...
    chartIds.forEach((chartId, index) => {
      ...
      if (index === 0) {
        y += 14;
        doc.setFontSize(13);
        doc.text(t("pdfSectionCharts"), margin, y);
        y += 14;
      }
      if (y > pageHeight - 220) {
        doc.addPage();
      ...
      doc.addImage(imageData, "PNG", margin, y, pageWidth - margin * 2, 180);
      y += 196;
    });
